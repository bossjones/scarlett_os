...
@pytest.mark.skip(
    reason="Broken till we get 99-pytest working, this will be broken unfortunately"
)
class TestScarlettListener(object):
    def test_scarlett_listener_interfaces(
        self, create_dummy_session_dbus, sl, main_loop
    ):
        assert (
            sl.__repr__() == "<ScarlettListener(org.scarlett, /org/scarlett/Listener)>"
        )
    ...
    def test_scarlett_listener_emit_methods(
        self, create_dummy_session_dbus, sl, main_loop
    ):
