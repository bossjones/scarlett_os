...
class TestUnitSystem(unittest.TestCase):
    ...
    def test_invalid_units(self):
        ...
        with self.assertRaises(ValueError):
            UnitSystem(
                SYSTEM_NAME, INVALID_UNIT, LENGTH_METERS, VOLUME_LITERS, MASS_GRAMS
            )

        with self.assertRaises(ValueError):
            UnitSystem(
                SYSTEM_NAME, TEMP_CELSIUS, INVALID_UNIT, VOLUME_LITERS, MASS_GRAMS
            )

        with self.assertRaises(ValueError):
            UnitSystem(
                SYSTEM_NAME, TEMP_CELSIUS, LENGTH_METERS, INVALID_UNIT, MASS_GRAMS
            )

        with self.assertRaises(ValueError):
            UnitSystem(
                SYSTEM_NAME, TEMP_CELSIUS, LENGTH_METERS, VOLUME_LITERS, INVALID_UNIT
            )
    ...
    def test_temperature_same_unit(self):
        ...
        self.assertEqual(
            5, METRIC_SYSTEM.temperature(5, METRIC_SYSTEM.temperature_unit)
        )
    ...
    def test_temperature_to_imperial(self):
        ...
        self.assertEqual(
            77, IMPERIAL_SYSTEM.temperature(77, IMPERIAL_SYSTEM.temperature_unit)
        )
        self.assertEqual(
            77, IMPERIAL_SYSTEM.temperature(25, METRIC_SYSTEM.temperature_unit)
        )
