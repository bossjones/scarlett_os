...
class TestScarlettCommand(object):
    ...
    def test_get_current_date(self):
        # t = TimeCommand
        with freeze_time("2016-11-23 17:34:39"):
            assert commands.TimeCommand.get_current_date() == "Today's date is, Wednesday, November 23, 2016"

    def test_check_cmd(self, commands_unit_mocker_stopall):
        # c = Command
        temp_mocker = commands_unit_mocker_stopall

        mock_check_cmd = temp_mocker.MagicMock(name='mock_check_cmd', return_value='It is now, 05:34 PM')
        ...
        assert commands.Command.check_cmd(command_tuple=(' ScarlettListener caught a command match', 'pi-response', 'what time is it',)) == 'It is now, 05:34 PM'
